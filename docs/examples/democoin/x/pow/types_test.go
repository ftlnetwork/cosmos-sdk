...
func TestMsgMineString(t *testing.T) {
	...
	res := msg.String()
	require.Equal(t, res, "MsgMine{Sender: ftlnet1wdjkuer9wgh76ts6, Difficulty: 0, Count: 0, Nonce: 0, Proof: abc}")
}
...
func TestMsgMineGetSignBytes(t *testing.T) {
	...
	require.Equal(t, string(res), `{"count":1,"difficulty":1,"nonce":1,"proof":"YWJj","sender":"ftlnet1wdjkuer9wgh76ts6"}`)
}
